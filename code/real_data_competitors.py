...
with open("/home/rmmeng/data/neural/mc_maze_nerual_spikes_smth_50_vs_handvel_cond0.pickle", "rb") as f:
    mc = pickle.load(f)
print("mc, input dim={}".format(mc["rates"].shape[-1]))
...
#                                    num_cv_folds=n_cv, decoding_window=win, n_init=n_init, verbose=True)
rate = mc["rates"]
rate = rate.reshape(-1, rate.shape[-1])
vels = mc["vels"]
vels = vels.reshape(-1, vels.shape[-1])
mc_results = analysis.run_analysis(rate, vels, T_pi_vals, dim_vals=mc_dims, offset_vals=offsets,
                                   num_cv_folds=n_cv, decoding_window=win, n_init=n_init, verbose=True)

# with open("res/m1_dca/result_cpt.pkl", "wb") as f:
#     pickle.dump(M1_results, f)
# with open("res/hc_dca/result_cpt.pkl", "wb") as f:
#     pickle.dump(HC_results, f)
# with open("res/temp_dca/result_cpt.pkl", "wb") as f:
#     pickle.dump(weather_results, f)
# with open("res/ms_dca/result_cpt.pkl", "wb") as f:
#     pickle.dump(ms_results, f)
with open("res/mc_pca_sfa_dca/result_cpt.pkl", "wb") as f:
    pickle.dump(mc_results, f)

print("average score: ", mc_results.mean(axis=0))
