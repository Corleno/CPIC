...
def plot_3d_colored(rf_coefficients_2norm, X, color="black", linewidth=2, ax=None, max_2norm=1, verbose=False):
    if ax is None:
        fig = plt.figure()
        ax = fig.add_subplot(111, projection='3d')

    color_matrix = rf_coefficients_2norm
    ax.plot(X[:, 0], X[:, 1], X[:, 2], c=color, linewidth=linewidth)
    p = ax.scatter(X[:, 0], X[:, 1], X[:, 2], c=color_matrix, cmap='jet', marker='8', linewidth=linewidth, vmin=0, vmax=max_2norm)
    if verbose:
        plt.colorbar(p)
        plt.show()
...
def plot_lorenz_3d_colored(ax, X, X_true, linewidth, max_2norm=1):
    mat_minus = X - X_true
